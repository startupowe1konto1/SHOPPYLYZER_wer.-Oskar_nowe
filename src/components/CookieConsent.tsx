...
export const CookieConsent = () => {
  ...
  const initFunctional = () => {
    /* WSTAW TUTAJ: inicjalizację Bubble.io lub narzędzia funkcjonalnego */
    ...
    <>
      ...
      {visible && (
        <div className="fixed bottom-0 left-0 right-0 z-50 flex justify-center px-4">
          <div className="w-full max-w-3xl rounded-t-2xl p-6" style={{ background: '#1E5F8E' }}>
            <h3 className="text-white font-bold text-base mb-2">Dbamy o Twój komfort pracy w Shoppalyzer</h3>
            <p className="text-white/85 text-sm leading-relaxed mb-4">
              Wykorzystujemy pliki cookies, aby nasz system działał stabilnie i bezpiecznie (ciasteczka niezbędne). Za Twoją zgodą my oraz nasi partnerzy (m.in. Bubble.io i Stripe) chcemy używać dodatkowych plików cookies, aby analizować ruch w aplikacji i zapamiętywać Twoje preferencje biznesowe. Szczegóły w{' '}
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={acceptAll} className="px-5 py-2.5 rounded-lg text-sm font-bold text-white" style={{ background: '#F0921C' }}>Akceptuję wszystkie</button>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="bg-white rounded-2xl p-8 w-full max-w-lg">
            <h3 className="text-xl font-bold mb-1" style={{ color: '#1E5F8E' }}>Zarządzanie ustawieniami prywatności</h3>
            <p className="text-sm text-gray-500 mb-5">Możesz dostosować swoje preferencje poniżej.</p>

            {/* Niezbędne */}
            <div className="border border-gray-200 rounded-xl p-4 mb-3">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm font-semibold" style={{ color: '#1E5F8E' }}>Niezbędne</span>
                <span className="text-xs font-semibold px-3 py-1 rounded-full" style={{ background: '#e5f3ff', color: '#1E5F8E' }}>Zawsze aktywne</span>
              </div>
            ...
            <div className="border border-gray-200 rounded-xl p-4 mb-3">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm font-semibold" style={{ color: '#1E5F8E' }}>Funkcjonalne</span>
            ...
            <div className="border border-gray-200 rounded-xl p-4 mb-3">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm font-semibold" style={{ color: '#1E5F8E' }}>Analityczne</span>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" className="sr-only peer" checked={analytics} onChange={e => setAnalytics(e.target.checked)} />
                  <div className="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:bg-orange-400 after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-5"></div>
                </label>
              </div>
              <p className="text-xs text-gray-500 leading-relaxed">Pomagają nam i dostawcy Bubble.io zbierać anonimowe statystyki, abyśmy wiedzieli, które funkcje Shoppalyzera ulepszać.</p>
            </div>

            <button onClick={savePrefs} className="w-full py-3 rounded-lg text-sm font-bold text-white mt-2" style={{ background: '#F0921C' }}>Zapisz moje preferencje</button>
          </div>
        </div>
      )}
